...
mod TestHashCash {
    ...
    fn is_hashcode_valid_assert(){
        let verif_true = is_hashcode_valid("00006CAF49510986FF0E25C85F2E3088",16);
        assert!(true,verif_true);
        let verif_false = is_hashcode_valid("10006CAF49510986FF0E25C85F2E3088",16);
        assert!(false,verif_false);
    }
    #[test]
    fn convert_to_binary_from_hex_assert(){
        let binary = convert_to_binary_from_hex("00");
        assert!("00000000",binary);
        let binary = convert_to_binary_from_hex("AB");
    ...
    fn format_seed_and_message_assert(){
        let msg = format_seed_and_message("000000000000034C","hello");
